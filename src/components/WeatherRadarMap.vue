...
import { generateIndonesiaGfsWindGrid, fetchOpenMeteoWindData, interpolateWindVectorField, getGfsWindVectorAt } from '../utils/gfsWindGrid';
...
interface DayTab { dayStr: string; dateStr: string; }
...
// ── Real Location Mock Weather Dataset ───────────────────────────────────────
interface GeoStation {
  name: string; lat: number; lng: number; temp: number;
  windSpeed: number; windDeg: number; rain: number; clouds: number; uv: number; waves: number;
}
...
function getLayerValueAtGeo(lat: number, lng: number, layerId: string): number {
  if (layerId === 'wind') {
    ...
    return Math.max(0, Math.min(1, interpolateLocationData(lat, lng, 'rain') / 60));
  } else if (layerId === 'temperature') {
    return Math.max(0, Math.min(1, (interpolateLocationData(lat, lng, 'temp') - 18) / 20));
  } else if (layerId === 'clouds' || layerId === 'satellite') {
    return Math.max(0, Math.min(1, interpolateLocationData(lat, lng, 'clouds') / 100));
  } else if (layerId === 'waves') {
    return Math.max(0, Math.min(1, interpolateLocationData(lat, lng, 'waves') / 5));
  }
  const { speed } = getGfsWindVectorAt(lat, lng);
  return Math.max(0, Math.min(1, speed / 40));
}
...
// ── Leaflet shallowRef instance (Zero Proxy Bugs) ────────────────────────────
...
const inspected = ref<{ lat: number; lng: number; temp: number; status: string; wind: string; rain: number; } | null>(null);
...
function drawField() {
  ...
  const northLat = bounds.getNorth(), southLat = bounds.getSouth();
  const westLng = bounds.getWest(), eastLng = bounds.getEast();
...
// ── GFS Grid Dataset (GRIB2 U/V Format) ───────────────────────────────────────
const gfsDataGrid = generateIndonesiaGfsWindGrid();

// ── Dynamic Proyeksi Vektor Partikel (U & V Component) ───────────────────────
...
function windVec(px: number, py: number): [number, number] {
  ...
  // Interpolasi Vektor U & V Fisik Rigorus Berdasarkan Koordinat Lat/Lng Riil
...
function initParticles(W: number, H: number) {
  ...
  for (let i = 0; i < N_PARTICLES; i++) {
    ...
    particles.push({ x: Math.random() * W, y: Math.random() * H, age: Math.floor(Math.random() * life), life });
  }
}
...
function scheduleRedraw() {
  ...
  requestAnimationFrame(() => {
    ...
    if (activeLayer.value.hasParticles && showParticles.value) {
      if (!animId) { lastTs = performance.now(); animId = requestAnimationFrame(animLoop); }
    } else {
...
function updateBasemap() {
  ...
  tileLayer = L.tileLayer(url, { maxZoom: 18, attribution: '' }).addTo(leafletMap.value);
}

function updateCityMarkers() {
  if (!leafletMap.value) return;
  if (!cityMarkersLayer) cityMarkersLayer = L.layerGroup().addTo(leafletMap.value);
  ...
  for (const st of REAL_GEO_STATIONS) {
    ...
    `;
    const icon = L.divIcon({ className: 'windy-city-marker', html, iconSize: [0, 0] });
...
function setupCanvas() {
  ...
  fieldCanvas = fCvs; fieldCtx = fCvs.getContext('2d');
  ...
  particleCanvas = pCvs; particleCtx = pCvs.getContext('2d');
  ...
  leafletMap.value.on('move zoom movestart moveend zoomstart zoomend drag', scheduleRedraw);
}
...
function initMap() {
  ...
  // Log GFS GRIB2 U/V vector header (Leaflet Velocity compatible)
  console.log('GFS GRIB2 Wind Vector Grid initialized:', gfsDataGrid[0].header);
  ...
  leafletMap.value.on('click', (e: L.LeafletMouseEvent) => {
    ...
    inspected.value = { lat, lng, temp, status: statusStr, wind: `${spd} kt – ${dirStr}`, rain: rainVal };
...
const selectLayer = (id: string) => { activeLayerId.value = id; scheduleRedraw(); };
...
onMounted(async () => {
  ...
  // Fetch real-time Open-Meteo wind vectors from requested API endpoint
  const openMeteoData = await fetchOpenMeteoWindData(-2.5, 118.0);
  console.log('Fetched Open-Meteo Live U/V components:', openMeteoData);
});
    ...
    <div class="bg-slate-50/95 dark:bg-slate-900/95 text-slate-800 dark:text-white px-4 py-3 flex flex-col gap-2.5 z-20 border-t border-slate-200/80 dark:border-slate-800/80 backdrop-blur-md transition-colors duration-200">
      <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <div class="flex items-center gap-2.5 flex-1 min-w-0">
          ...
          <input type="range" min="0" :max="totalSlotsCount - 1" v-model.number="currentSlot" @input="scheduleRedraw()" class="w-full h-1.5 accent-blue-600 cursor-pointer rounded-lg bg-slate-200 dark:bg-slate-800" />
        </div>
        <div class="flex items-center bg-slate-200/70 dark:bg-slate-800/90 border border-slate-300/80 dark:border-slate-700/60 rounded-[4px] p-1 gap-1">
          <button v-for="m in forecastModels" :key="m.id" @click="activeModel = m.id; scheduleRedraw()"
            class="px-2.5 py-1 text-[10px] font-extrabold rounded-lg transition-all" :class="activeModel === m.id ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-300/60 dark:hover:bg-slate-700/60'">
            <span>{{ m.name }}</span>
            <span class="text-[8px] opacity-75 ml-1">{{ m.res }}</span>
          </button>
        </div>
      </div>
