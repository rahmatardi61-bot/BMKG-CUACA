...
// ── Daily weather detail (7 hari) aggregated from hourly forecasts ──────────
const dailyIconMap: Record<string, any> = { Sun, SunDim, Cloud, CloudRain, Moon };
...
const dailyForecasts = computed(() => {
  ...
  return [...byDate.entries()].slice(0, 7).map(([date, slots]) => {
    const midday = slots.find(s => s.time === '12:00') || slots[Math.floor(slots.length / 2)];
    const temps = slots.map(s => s.temp);
    return {
      label: new Date(date + 'T00:00:00').toLocaleDateString('id-ID', { weekday: 'short' }),
      icon: midday.icon,
      status: midday.status,
      min: Math.min(...temps),
      max: Math.max(...temps),
...
<template>
  <div class="space-y-6">
    ...
    >
      ...
      <!-- Daily Weather Detail Strip (7 hari) -->
      <div v-if="dailyForecasts.length" class="relative z-10 grid grid-cols-4 sm:grid-cols-7 gap-2 pt-4 border-t border-white/15">
        <div
          v-for="(d, i) in dailyForecasts"
          :key="d.label + i"
          class="flex flex-col items-center gap-1.5 rounded-xl px-1.5 py-2.5 bg-white/8 dark:bg-black/15 backdrop-blur-sm border border-white/10 transition-all duration-300 hover:bg-white/15"
          :class="d.isToday ? 'ring-1 ring-amber-300/70' : ''"
          :title="d.status"
        >
          <span class="text-[9px] font-black uppercase tracking-wider opacity-80">{{ d.label }}{{ d.isToday ? ' •' : '' }}</span>
          <component :is="dailyIconMap[d.icon] || Cloud" class="w-5 h-5" />
          <span class="text-[10px] font-black leading-none">{{ d.max }}°</span>
          <span class="text-[9px] font-bold opacity-60 leading-none">{{ d.min }}°</span>
        </div>
      </div>
