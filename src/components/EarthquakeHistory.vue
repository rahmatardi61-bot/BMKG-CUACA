...
const latestAutogempa = ref<EarthquakeEvent | null>(null);

const shortCity = computed(() => {
  if (!props.selectedCity) return 'lokasi Anda';
  const parts = props.selectedCity.split(',');
  return parts[0].trim();
});

// --- Inline Interactive Map State & Handlers ---
const inlineMapEl = ref<HTMLElement | null>(null);
let inlineMap: L.Map | null = null;
let inlineTileLayer: L.TileLayer | null = null;
let inlineMarker: L.Marker | null = null;
let inlineRipples: L.Circle[] = [];

const updateInlineMapTheme = () => {
  if (!inlineMap) return;
  ...
  if (inlineTileLayer) {
    inlineMap.removeLayer(inlineTileLayer);
  }

  inlineTileLayer = L.tileLayer(selectedUrl, {
    maxZoom: 18,
    attribution: '&copy; OpenStreetMap'
  }).addTo(inlineMap);
};

const updateInlineMapContent = () => {
  if (!inlineMap || !latestGlobalEvent.value) return;
  const event = latestGlobalEvent.value;
  ...
  inlineRipples.forEach(circle => inlineMap?.removeLayer(circle));
  inlineRipples = [];

  // Single subtle shockwave radius circle
  ...
  inlineRipples.push(circle);

  // Clean, authentic epicenter pulse marker
  const pulseHtml = `
    <div class="relative flex items-center justify-center">
      <span class="animate-ping absolute inline-flex h-8 w-8 rounded-full bg-red-500 opacity-40"></span>
      <div class="relative w-4 h-4 rounded-full bg-red-600 border-2 border-white shadow-md flex items-center justify-center">
  ...
  const epicIcon = L.divIcon({
    html: pulseHtml,
    className: 'custom-inline-epicenter',
    iconSize: [32, 32],
    iconAnchor: [16, 16]
  });

  inlineMarker = L.marker([event.lat, event.lng], { icon: epicIcon })
    .addTo(inlineMap)
    .bindPopup(`
      <div style="padding: 2px 4px; font-family: inherit; font-size: 11px;">
        <strong style="color: #ef4444; display: block; margin-bottom: 2px;">M ${event.magnitude.toFixed(1)}</strong>
        <span style="font-weight: 600;">${event.epicenter}</span>
        <div style="font-size: 10px; color: #64748b; margin-top: 4px;">Kedalaman: ${event.depth} km</div>
      </div>
    `);
};

const initInlineMap = () => {
  if (!inlineMapEl.value || !latestGlobalEvent.value) return;

  const event = latestGlobalEvent.value;

  if (inlineMap) {
    inlineMap.setView([event.lat, event.lng], 7);
    updateInlineMapTheme();
    updateInlineMapContent();
    inlineMap.invalidateSize();
    return;
  }

  inlineMap = L.map(inlineMapEl.value, {
    zoomControl: false,
...
const resetInlineMapCenter = () => {
  if (inlineMap && latestGlobalEvent.value) {
    inlineMap.flyTo([latestGlobalEvent.value.lat, latestGlobalEvent.value.lng], 7, {
      duration: 1.2
    });
  }
};

const destroyInlineMap = () => {
  if (inlineMap) {
    inlineMap.off();
    inlineMap.remove();
    inlineMap = null;
...
onMounted(async () => {
  try {
    ...
    const [autogempaRes, listRes] = await Promise.allSettled([
      fetchLatestAutogempa(),
      fetchRealtimeEarthquakes()
    ]);
    if (autogempaRes.status === 'fulfilled' && autogempaRes.value) {
      latestAutogempa.value = autogempaRes.value;
    }
    if (listRes.status === 'fulfilled') {
      earthquakesList.value = listRes.value;
    }
  } catch (error) {
...
const latestGlobalEvent = computed<HistoryEvent | null>(() => {
  const target = latestAutogempa.value || (earthquakesList.value.length > 0 ? earthquakesList.value[0] : null);
  if (!target) return null;
  const cityCoords = getCityCoordinates(props.selectedCity, props.userLat, props.userLng);
  const dist = calculateDistance(cityCoords.lat, cityCoords.lng, target.lat, target.lng);
  return {
    ...target,
    distance: dist
  };
...
const formatEpicenterHeadline = (epicenter?: string) => {
  if (!epicenter) return 'di Wilayah Indonesia';
  const cleaned = epicenter.replace(/^Pusat\s+gempa\s+berada\s+(di\s+)?/i, '').trim();
  if (!cleaned) return 'di Wilayah Indonesia';
  return `di ${cleaned.charAt(0).toUpperCase() + cleaned.slice(1)}`;
};
...
<template>
  ...
  >
    ...
    <!-- ─── GEMPA BUMI TERKINI (Clean Human Design Layout) ─── -->
    <div 
      v-if="latestGlobalEvent"
      class="bg-white dark:bg-[#070c19] border border-slate-200 dark:border-slate-800 rounded-xl p-5 mb-5 transition-all relative z-10 shadow-xs"
    >
      <div class="flex flex-col md:flex-row gap-6 items-stretch">
        
        <!-- PETA LOKASI EPISENTER (Sebelah Kiri) -->
        <div class="w-full md:w-[280px] lg:w-[300px] shrink-0 self-stretch flex flex-col">
          <div class="relative w-full h-[250px] md:h-full min-h-[270px] rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 shadow-inner flex flex-col justify-between group">
            
            <!-- Leaflet Container -->
            <div 
              ref="inlineMapEl" 
              class="w-full h-full z-0 cursor-grab active:cursor-grabbing"
            ></div>

            <!-- Top Right Map Re-center -->
            <button 
              type="button" 
              @click="resetInlineMapCenter"
              title="Pusatkan Peta ke Episenter"
              class="absolute top-2.5 right-2.5 z-[400] p-1.5 rounded-lg bg-white/90 dark:bg-slate-900/90 hover:bg-white text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-slate-700/80 shadow-xs transition-all cursor-pointer backdrop-blur-xs active:scale-95"
            >
              <RotateCcw class="w-3.5 h-3.5" />
            </button>

            <!-- Bottom Floating Badge: Coordinates -->
            <div class="absolute bottom-2.5 left-2.5 z-[400] pointer-events-auto">
              <span class="px-2 py-1 rounded-md bg-white/90 dark:bg-slate-900/90 backdrop-blur-xs border border-slate-200/80 dark:border-slate-800 text-[10px] font-mono font-bold text-slate-700 dark:text-slate-300 shadow-xs">
                {{ latestGlobalEvent.lintang || `${Math.abs(latestGlobalEvent.lat).toFixed(2)} LS` }} • {{ latestGlobalEvent.bujur || `${Math.abs(latestGlobalEvent.lng).toFixed(2)} BT` }}
              </span>
            </div>
          </div>
        </div>

        <!-- INFORMASI GEMPA BUMI (Sebelah Kanan - Proper & Balanced) -->
        <div class="flex-1 min-w-0 flex flex-col justify-between w-full space-y-3.5">
          <div>
            <!-- Top Metadata Bar: Time on Left, Status Pills on Right -->
            <div class="flex flex-wrap items-center justify-between gap-2 pb-2.5 border-b border-slate-100 dark:border-slate-800/60">
              <div class="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-semibold">
                <span class="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                <span>{{ latestGlobalEvent.date }} • {{ latestGlobalEvent.time.replace(/WIB/gi, '').trim() }} WIB</span>
              </div>

              <div class="flex items-center gap-2">
                <span 
                  class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border"
                  :class="latestGlobalEvent.tsunamiPotential 
                    ? 'bg-red-50 dark:bg-red-950/60 text-red-700 dark:text-red-300 border-red-200/80 dark:border-red-700/50' 
                    : 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border-emerald-200/80 dark:border-emerald-700/50'"
                >
                  <span class="w-1.5 h-1.5 rounded-full" :class="latestGlobalEvent.tsunamiPotential ? 'bg-red-500 animate-ping' : 'bg-emerald-500'"></span>
                  <span>{{ latestGlobalEvent.tsunamiPotential ? 'Potensi Tsunami' : 'Tidak Berpotensi Tsunami' }}</span>
                </span>
                <span 
                  v-if="latestGlobalEvent.dirasakan || (latestGlobalEvent.feltMmi && latestGlobalEvent.feltMmi.length > 0)"
                  class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-slate-100 dark:bg-slate-800/90 text-slate-700 dark:text-slate-200 border border-slate-200/70 dark:border-slate-700/70"
                >
                  Gempa Dirasakan
                </span>
              </div>
            ...
            <div class="mt-3 mb-1.5">
              <h3 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white leading-tight">
                Gempa M {{ latestGlobalEvent.magnitude.toFixed(1).replace('.', ',') }} {{ formatEpicenterHeadline(latestGlobalEvent.epicenter) }}
              </h3>
              <p class="text-xs sm:text-[13px] text-slate-500 dark:text-slate-400 mt-1.5 leading-relaxed">
                {{ latestGlobalEvent.epicenter }} <span class="text-slate-300 dark:text-slate-700 mx-1">•</span> Berjarak ±{{ latestGlobalEvent.distance }} km dari {{ shortCity }}
              </p>
            </div>

            <!-- 3 Clean Structured Parameter Cards -->
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 my-3.5">
              <!-- Magnitudo / Kekuatan -->
              <div class="eq-param-card p-2.5 sm:p-3 rounded-xl border flex flex-col justify-between transition-colors">
                <div class="flex items-center justify-between gap-1.5 mb-1.5">
                  <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 leading-none">Kekuatan</span>
                  <div class="w-5 h-5 rounded-md bg-red-500/10 dark:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/20 dark:border-red-500/30 flex items-center justify-center shrink-0">
                    <Activity class="w-3 h-3" />
                  </div>
                </div>
                <div class="text-xs sm:text-[13px] font-bold text-slate-800 dark:text-white leading-snug">
                  M {{ latestGlobalEvent.magnitude.toFixed(1).replace('.', ',') }}
                  <span class="text-[11px] font-semibold text-slate-600 dark:text-slate-200">({{ latestGlobalEvent.magnitude >= 5 ? 'Signifikan' : 'Ringan' }})</span>
                </div>
              </div>
              ...
              <div class="eq-param-card p-2.5 sm:p-3 rounded-xl border flex flex-col justify-between transition-colors">
                <div class="flex items-center justify-between gap-1.5 mb-1.5">
                  <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 leading-none">Kedalaman</span>
                  <div class="w-5 h-5 rounded-md bg-emerald-500/10 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 dark:border-emerald-500/30 flex items-center justify-center shrink-0">
                    <Compass class="w-3 h-3" />
                  </div>
                </div>
                <div class="text-xs sm:text-[13px] font-bold text-slate-800 dark:text-white leading-snug">
                  {{ latestGlobalEvent.depth }} km
                  <span class="text-[11px] font-semibold text-slate-600 dark:text-slate-200">({{ latestGlobalEvent.depth <= 70 ? 'Dangkal' : 'Menengah' }})</span>
                </div>
              </div>
              ...
              <div class="eq-param-card p-2.5 sm:p-3 rounded-xl border flex flex-col justify-between transition-colors">
                <div class="flex items-center justify-between gap-1.5 mb-1.5">
                  <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 leading-none">Koordinat</span>
                  <div class="w-5 h-5 rounded-md bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 border border-blue-500/20 dark:border-blue-500/30 flex items-center justify-center shrink-0">
                    <MapPin class="w-3 h-3" />
                  </div>
                </div>
                <div class="text-xs font-bold text-slate-800 dark:text-white font-mono leading-snug">
                  {{ latestGlobalEvent.lintang || `${Math.abs(latestGlobalEvent.lat).toFixed(2)} LS` }} • {{ latestGlobalEvent.bujur || `${Math.abs(latestGlobalEvent.lng).toFixed(2)} BT` }}
                </div>
              </div>
            </div>

            <!-- Imbauan & Panduan Warga (Simple, Clean, Human) -->
            <div class="eq-mitigation-box p-3 rounded-xl border flex items-start gap-2.5 text-left transition-colors">
              <ShieldCheck class="w-4 h-4 text-sky-600 dark:text-sky-400 shrink-0 mt-0.5" />
              <p class="text-xs text-slate-700 dark:text-slate-200 leading-relaxed">
                <span class="font-bold text-slate-900 dark:text-white">Arahan Mitigasi:</span> 
                {{ latestGlobalEvent.potensi ? `${latestGlobalEvent.potensi}. Pastikan tempat tinggal aman dan periksa jalur evakuasi di sekitar Anda.` : 'Masyarakat diimbau tetap tenang dan waspada terhadap potensi gempa susulan. Pastikan tempat tinggal aman dan periksa jalur evakuasi di sekitar Anda.' }}
              </p>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/60">
            <button 
              ...
              class="inline-flex items-center gap-1.5 text-xs font-bold text-blue-600 dark:text-sky-400 hover:text-blue-700 dark:hover:text-sky-300 transition-colors cursor-pointer group text-left"
            >
              <span class="text-left">Parameter Lengkap &amp; Wilayah Terdampak</span>
            ...
            <button 
              type="button"
              @click="openMapModal"
              class="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors cursor-pointer text-left"
            >
              <Globe class="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <span class="text-left">Peta Seismik Nasional</span>
            </button>
          </div>
        </div>
    ...
    >
      ...
      <div class="space-y-1 flex-1 min-w-0">
        <div class="flex flex-wrap items-center gap-1.5">
          <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-current/10 text-current">
            <MapPin class="w-2.5 h-2.5 shrink-0" />
            Berdasarkan Lokasi: {{ shortCity }}
          </span>
...
:deep(.custom-inline-epicenter) {
  background: transparent !important;
  border: none !important;
}

:deep(.city-anchor-marker) {
  background: transparent !important;
  border: none !important;
}
</style>
