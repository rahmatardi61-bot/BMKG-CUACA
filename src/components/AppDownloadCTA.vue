...
<template>
  ...
  <div class="cta-card relative rounded-3xl overflow-hidden min-h-[280px] lg:min-h-0 shadow-lg group/cta cursor-default w-full h-full flex flex-col">
    ...
    <!-- ─── Content ─── -->
    <div class="relative h-full flex flex-col justify-between p-5 sm:p-6 text-left z-10" style="min-height: inherit;">
      ...
      <div class="flex flex-col gap-3.5">
        <div class="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/70 dark:bg-white/15 backdrop-blur-md border border-slate-200/50 dark:border-white/20 shadow-sm dark:shadow-white/5 p-2 select-none">
          <img :src="logoSvg" alt="Info BMKG Logo" class="w-full h-full object-contain" />
        </div>
        <div class="mt-0.5">
          ...
          <p class="text-[10px] sm:text-xs font-medium leading-relaxed text-slate-500 dark:text-slate-400 mt-2 max-w-[170px]">
      ...
      <div class="mt-auto pt-6 flex justify-center w-full">
...
/* ── CTA Card — Dark Mode ── */
.dark .cta-card {
  background-color: rgba(11, 22, 48, 0.6) !important;
  backdrop-filter: blur(12px);
